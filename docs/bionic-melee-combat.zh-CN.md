...
普通和高级智力选臂时会优先考虑装配方向与目标方向的匹配程度，低处优先下垂手臂、高处优先上举手臂，同侧目标优先侧伸手臂；恢复状态、武器与轮换仍参与选择。姿态匹配权重固定为 1.0。简单智力固定使用基础 DPS 最高的一只手臂。
...
智力通过 `bionic_head/intelligence` 数据包分类读取，多头取最高级，无头或未分类时为简单级。简单智力在全部手臂中固定选择单臂基础 DPS 最高的一只，按伤害倍率 × 20 / 单臂恢复间隔比较，不计武器、附魔和状态效果；相同 DPS 按右侧槽位、左侧槽位顺序取第一只。该臂冷却中、被遮挡或无法覆盖目标时不会换臂，协调计数只计这一只。普通智力保留原有选臂与停步攻击行为。高级智力沿用普通智力的选臂，攻击全程继续寻路追击，步行和跳跃移动都不因挥击强制停止；接触阶段仍锁定攻击方向，移动只平移攻击区域。所有等级共用相同的伤害、范围、恢复、转身及准备期瞄准参数。

## 攻击生命周期

1. 全局冷却结束、目标有效且可见时，选择符合条件的手臂。
2. 立即记录全局及该手臂的下次可用游戏时间。即使落空、被格挡、目标离开或 AI 中断，这些时间也不清零。
3. 无肘手臂以 `clamp(round(全局间隔 × 0.2), 3, 6) + 2` tick 定位窗口；有肘手臂按实际播放长度，将空手动画的第 15/25 曲线 tick 或武器动画的 0.5833/1.125 秒关键帧换算并四舍五入，再整体前移 4 tick。准备期间有限追踪；简单和普通智力在整个攻击动作期间暂停主动追赶，高级智力继续追击。
4. 接触窗口持续 5 tick，使用锁定方向判定。首次接触只调用一次伤害流程，即使伤害被盾牌、无敌帧或事件拒绝也不会重复调用。
5. 接触窗口结束后继续完成动作；简单和普通智力在动作结束后恢复追赶，高级智力保持追击，并等待全局与手臂冷却。换目标会取消旧动作，但不会把旧动作转移到新目标或刷新冷却。
