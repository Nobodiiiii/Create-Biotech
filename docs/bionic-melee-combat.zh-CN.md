...
- 命中区域是锁定方向左右各 35°、上下各 45° 的扇区，与装配活动范围、身体前半空间、肩部半径为 `reach` 的球体取交集。所有姿态都钳制在身体正前方 180°（左右各 90°）以内。
...
| --- | --- | --- |
| 下垂 | 向下 70°、向上 35° | 身体正前方 180° |
| 向前平伸 | 向下 50°、向上 50° | 身体正前方 180° |
| 上举 | 向下 35°、向上 80° | 身体正前方 180° |
| 向侧面平举 | 向下 50°、向上 50° | 向本侧偏置 20° 后钳制到前方，跨身一侧收窄为 70°，本侧保留 90° |

参数按装配单位方向的垂直、横向分量连续插值，斜向装配没有分类跳变。瞄准中心先钳制到允许方向，再检查目标完整 AABB，因此目标仅部分进入活动范围仍可命中。所有姿态都能覆盖正前方同高度目标；有效臂长和伤害不因姿态改变。
...
客户端接收攻击开始、准备期间的追踪、目标点锁定和取消事件；方向与肩部到目标碰撞箱最近点的距离会一起更新，同时校正逻辑动作的剩余时间而不重启动画。逻辑动作时长为 5–8 tick，通常动画播放 15 tick，播放速度由全局间隔选择。攻击手不再使用固定肩肘关键帧决定落点：单段手臂直接朝向目标点，带肘手臂按真实上下臂长度进行两段 IK，在接触前逐渐抵达目标、接触期间保持、随后回到常态；躯干和另一只手仍保留配重动作。服务端命中仍不取样动画，动画目标点也会钳制在该手臂的物理可达距离内。
