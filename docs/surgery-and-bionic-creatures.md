...
The server controls contact and recovery. Articulated attacks convert the same authored impact keyframe used by the animation into their server damage-window position, while rigid attacks retain their earlier independent position. The visible hand path never changes attack reach.
