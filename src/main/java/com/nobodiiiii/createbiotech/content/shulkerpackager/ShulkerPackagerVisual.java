...
public class ShulkerPackagerVisual extends AbstractBlockEntityVisual<ShulkerPackagerBlockEntity>
	...
	public ShulkerPackagerVisual(VisualizationContext ctx, ShulkerPackagerBlockEntity blockEntity, float partialTick) {
		super(ctx, blockEntity, partialTick);

		lastHatchPartial =
			ShulkerPackagerRenderer.getHatchModel(blockEntity.animationInward, blockEntity.animationTicks);
		hatch = instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(lastHatchPartial))
			.createInstance();

		tray = instancerProvider()
			.instancer(InstanceTypes.TRANSFORMED, Models.partial(ShulkerPackagerRenderer.getTrayModel(blockState)))
			.createInstance();
