...
import static com.nobodiiiii.createbiotech.content.magmabelt.transport.MagmaBeltTunnelInteractionHandler.flapTunnel;
...
public class MagmaBeltInventory {

	final MagmaBeltBlockEntity belt;
	...
	public void tick() {
		...
		boolean horizontal = belt.getBlockState()
			.getValue(MagmaBeltBlock.SLOPE) == BeltSlope.HORIZONTAL;
		float spacing = 1;
		...
		while (iterator.hasNext()) {
			...
			if (!onClient && handleMagmaBlastingAndCheckIfRemoved(currentItem)) {
				iterator.remove();
				currentItem = null;
				continue;
			}
			...
			// Belt Funnels
			if (MagmaBeltFunnelInteractionHandler.checkForFunnels(this, currentItem, nextOffset))
				continue;
			...
			// Belt Tunnels
			if (MagmaBeltTunnelInteractionHandler.flapTunnelsAndCheckIfStuck(this, currentItem, nextOffset))
				continue;

			// Horizontal Crushing Wheels
			if (MagmaBeltCrusherInteractionHandler.checkForCrushers(this, currentItem, nextOffset))
				continue;
			...
			BlockPos nextPosition = MagmaBeltHelper.getPositionForOffset(belt, beltMovementPositive ? belt.beltLength : -1);
	...
	private void tickMagmaBlastingWithoutMovement() {
		...
		while (iterator.hasNext()) {
			...
			if (handleMagmaBlastingAndCheckIfRemoved(currentItem)) {
				iterator.remove();
				belt.notifyUpdate();
			}
		}
	...
	protected boolean handleMagmaBlastingAndCheckIfRemoved(TransportedItemStack currentItem) {
		...
		if (!AllFanProcessingTypes.BLASTING.canProcess(currentItem.stack, belt.getLevel())) {
			if (currentItem.processedBy == AllFanProcessingTypes.BLASTING) {
				currentItem.clearFanProcessingData();
		...
		boolean wasProcessing = currentItem.processedBy == AllFanProcessingTypes.BLASTING;
		...
		if (result.doesNothing()) {
			if (!wasProcessing || !ItemStack.matches(currentItem.stack, stackBefore))
				belt.notifyUpdate();
			else
				belt.setChanged();
			return false;
		...
		if (result.hasHeldOutput()) {
			...
			prepareMagmaBlastingOutput(currentItem, held);
			toInsert.add(held);
		}

		for (TransportedItemStack output : result.getOutputs()) {
			prepareMagmaBlastingOutput(currentItem, output);
			toInsert.add(output);
		}
	...
	private void prepareMagmaBlastingOutput(TransportedItemStack source, TransportedItemStack output) {
		output.beltPosition = source.beltPosition;
		output.prevBeltPosition = source.prevBeltPosition;
		output.sideOffset = source.sideOffset;
		output.prevSideOffset = source.prevSideOffset;
	...
	protected boolean handleBeltProcessingAndCheckIfRemoved(TransportedItemStack currentItem, float nextOffset,
		...
		if (currentItem.beltPosition > .5f || beltMovementPositive) {
			...
			for (int segment = firstUpcomingSegment; beltMovementPositive ? segment + .5f <= nextOffset
				...
				if (stackHandlerBehaviour == null)
					continue;
				if (BeltProcessingBehaviour.isBlocked(belt.getLevel(), MagmaBeltHelper.getPositionForOffset(belt, segment)))
					continue;
	...
	protected BeltProcessingBehaviour getBeltProcessingAtSegment(int segment) {
		return BlockEntityBehaviour.get(belt.getLevel(), MagmaBeltHelper.getPositionForOffset(belt, segment)
...
//			return Ending.FUNNEL;
	...
	public void eject(TransportedItemStack stack) {
		...
		Vec3 outPos = MagmaBeltHelper.getVectorForOffset(belt, stack.beltPosition);
