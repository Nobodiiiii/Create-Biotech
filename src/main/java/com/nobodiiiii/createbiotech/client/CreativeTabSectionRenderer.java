...
public final class CreativeTabSectionRenderer {
	...
	private static Map<CBCreativeTabSection, Integer> activeSectionRows = Map.of();
	...
	public static Collection<ItemStack> activateLayout(Collection<ItemStack> displayItems) {
		CBCreativeModeTabs.CreativeTabScreenLayout layout =
			CBCreativeModeTabs.createMainTabScreenLayout(displayItems);
		activeSectionRows = layout.sectionRows();
		currentRow = 0;
		return layout.items();
	}

	public static void resetLayout() {
		activeSectionRows = Map.of();
		currentRow = 0;
	}

	public static void render(CreativeModeInventoryScreen screen, GuiGraphics graphics) {
		if (activeSectionRows.isEmpty())
			return;
		...
		try {
			...
			for (CBCreativeTabSection section : CBCreativeTabSection.values()) {
				int sectionRow = activeSectionRows.getOrDefault(section, -1) - currentRow;
